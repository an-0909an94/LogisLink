	...
	<div class="contents">
		<div id="group-list" class="cont-wrapper-page-grid">
			...
			<div class="form-group row">
				<div class="input-group input-group-sm col-1 middle-name">
				<strong>톤급</strong>
				    <select class="custom-select col-12" id="selTon" name="selTon">
				    </select>
				</div>
				<c:if test="${authChk}">
				<div class="input-group input-group-sm col-1 middle-name">
...
oGrid.initGrid();
oGrid.setSendUrl("/contents/statistics/data/statLoadGrid.do");
...
function createChart(param) {
	$("#chart").kendoChart({
    	dataSource: {
            transport: {
                read: {
                    url: "/contents/statistics/data/statLoad.do",
                    type : "post",
                    dataType: "json",
                    data : param,
        ...
        title: {
            text: "적재율 통계 (" + moment(param.fromDate).format('YYYY-MM-DD') + " ~ " + moment(param.toDate).format('YYYY-MM-DD') + ")"
        },
        legend: {
            position: "bottom"
        },
