...
function goList(){
	...
	columns = [
		{ field: "custName", title: "거래처", width: 173.5, locked: true},
		{ field: "type", title: "구분", width: 100, locked: true},
		{ field: "colSum", title: "소계", width: 100, locked: true,
			attributes: { 
				style: "text-align: right" 
			}
		},
		{ field: "custId", title: "custId", locked: true, hidden: true},
		{ field: "deptId", title: "deptId", locked: true, hidden: true}
	];
			...
			schema : {
				data : function(response) {
					...
					for(var i=0, item; item=response.cust[i]; i++) {
						sellData = {"custName" : item.custName, "type" : "매출", "custId" : item.custId, "deptId" : item.deptId};
						buyData = {"custName" : item.custName, "type" : "매입", "custId" : item.custId, "deptId" : item.deptId};
						profData = {"custName" : item.custName, "type" : "이익", "custId" : item.custId, "deptId" : item.deptId};
						percData = {"custName" : item.custName, "type" : "%", "custId" : item.custId, "deptId" : item.deptId};
						sellSum = 0; 
						buySum = 0;
						...
						for(var j=0, item2; item2=response.data[j]; j++) {	
							if(item.custId == item2.custId && item.deptId == item2.deptId) {
								var month = item2.allocDate.substring(7, 5);
								sellData["col" + month] = Util.nvl(Util.formatNumber(item2.sellAmt),"0");
								buyData["col" + month] = Util.nvl(Util.formatNumber(item2.buyAmt),"0");
								profData["col" + month] = Util.nvl(Util.formatNumber(item2.sellAmt - item2.buyAmt),"0");
								var perData = 0;
								if (item2.sellAmt != item2.buyAmt && item2.sellAmt != 0) perData = (item2.sellAmt - item2.buyAmt) / item2.sellAmt * 100;
								percData["col" + month] = perData.toFixed(1)+"%";
								sellSum += item2.sellAmt;
								buySum += item2.buyAmt;
							}
						}
						sellData["colSum"] = Util.nvl(Util.formatNumber(sellSum), "0");
						buyData["colSum"] =  Util.nvl(Util.formatNumber(buySum), "0");
						profData["colSum"] = Util.nvl(Util.formatNumber(sellSum - buySum), "0");
						if (sellSum != buySum  && sellSum != 0) {
							percData["colSum"] = (sellSum - buySum) / sellSum * 100;
						} else {
							percData["colSum"] = 0
						}					
						percData["colSum"] = percData["colSum"].toFixed(1)+"%"; 
	...
	var gridCellClassName;
  	for (var i = 0; i < rows.length; i++) {
    	...
    	var row2= $(rows2[i]);
   		...
   		cells.addClass(gridAddClass(cells.prevObject[3].innerText+"-"+cells.prevObject[4].innerText));
