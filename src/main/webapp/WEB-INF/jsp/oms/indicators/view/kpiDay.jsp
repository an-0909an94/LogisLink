...
function goList(){
	...
	$("#grid").kendoGrid({
		dataSource : {
			transport : {
				read : {
					url : "/oms/indicators/data/kpiDay.do",
					type : "post",
					dataType : "json",
			...
			schema : {
				data : function(response) {
					...
					for (var item, i=0; item = response.data[i]; i++) {
						...
						if(item.preOrder != "0"){
							preData["col" + day] = Util.formatNumber(item.preOrder + "</br>" + (item.preOrder / orderSum * 100).toFixed(2) + "%");
						}else{
							preData["col" + day] = Util.formatNumber(item.preOrder + "</br>" + "0.00%");
						}
						
						if(item.postOrder != "0"){
							postData["col" + day] = Util.formatNumber(item.postOrder + "</br>" + (item.postOrder / orderSum * 100).toFixed(2) + "%");
						}else{
							postData["col" + day] = Util.formatNumber(item.postOrder + "</br>" + "0.00%");
						}
						
						if(item.todayFinish != "0"){
							todayData["col" + day] = Util.formatNumber(item.todayFinish + "</br>" + (item.todayFinish / (item.tomorrowFinish + item.todayFinish) * 100).toFixed(2) + "%");
						}else{
							todayData["col" + day] = Util.formatNumber(item.todayFinish + "</br>" + "0.00%");
						}
						
						if(item.tomorrowFinish != "0"){
							tomorrowData["col" + day] = Util.formatNumber(item.tomorrowFinish + "</br>" + (item.tomorrowFinish / (item.tomorrowFinish + item.todayFinish) * 100).toFixed(2) + "%");
						}else{
							tomorrowData["col" + day] = Util.formatNumber(item.tomorrowFinish + "</br>" + "0.00%");
						}
						
						if(item.allocDelay != "0"){
							allocData["col" + day] = Util.formatNumber(item.allocDelay + "</br>" + (item.allocDelay / orderSum * 100).toFixed(2) + "%");
						}else{
							allocData["col" + day] = Util.formatNumber(item.allocDelay + "</br>" + "0.00%");
						}
						
						if(item.enterDelay != "0"){
							enterData["col" + day] = Util.formatNumber(item.enterDelay + "</br>" + (item.enterDelay / orderSum * 100).toFixed(2) + "%");
						}else{
							enterData["col" + day] = Util.formatNumber(item.enterDelay + "</br>" + "0.00%");
						}
						
						if(item.finishDelay != "0"){
							finishData["col" + day] = Util.formatNumber(item.finishDelay + "</br>" + (item.finishDelay / orderSum * 100).toFixed(2) + "%");
						}else{
							finishData["col" + day] = Util.formatNumber(item.finishDelay + "</br>" + "0.00%");
						}
					}
		...
		excel: {
			fileName: "KPI(일)(" + new Date().yyyymmdd() + ").xlsx",
