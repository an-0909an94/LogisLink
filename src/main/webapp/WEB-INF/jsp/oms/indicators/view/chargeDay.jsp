...
function goList(){
	...
	for(var i = 1; i<=lastDay; i++) {
		...
		data = {field: colName, title: title, width: 80, encoded: false};
	...
	$("#grid").kendoGrid({
		dataSource : {
			transport : {
				read : {
					url : "/oms/indicators/data/chargeDay.do",
					type : "post",
					dataType : "json",
			...
			schema : {
				data : function(response) {
					...
					for (var item, i=0; item = response.data[i]; i++) {
						var day = item.sDate.substring(10,8);
						var tonSum = 0;
						var amtSum = 0;
						tonSum += item.cnt1T,
						tonSum += item.cnt2T5;
						...
						amtSum += item.amt40FT;
						amtSum += item.amtETC;
						data["col" + day] = Util.formatNumber(item.orderCnt);
						...
						dataTonSum["col" + day] = Util.formatNumber(tonSum);
						dataTonSum["subtotal"] = dataTonSum["subtotal"] + tonSum;
						data1T["col" + day] = Util.nvl(Util.formatNumber(item.cnt1T), 0);
						data1T["subtotal"] = data1T["subtotal"] + item.cnt1T;
						data2T5["col" + day] = Util.nvl(Util.formatNumber(item.cnt2T5), 0);
						data2T5["subtotal"] = data2T5["subtotal"] + item.cnt2T5;
						data3T5["col" + day] = Util.nvl(Util.formatNumber(item.cnt3T5), 0);
						data3T5["subtotal"] = data3T5["subtotal"] + item.cnt3T5;
						data5T["col" + day] = Util.nvl(Util.formatNumber(item.cnt5T), 0);
						data5T["subtotal"] = data5T["subtotal"] + item.cnt5T;
						data5TA["col" + day] = Util.nvl(Util.formatNumber(item.cnt5A), 0);
						data5TA["subtotal"] = data5TA["subtotal"] + item.cnt5A;
						data11T["col" + day] = Util.nvl(Util.formatNumber(item.cnt11T), 0);
						data11T["subtotal"] = data11T["subtotal"] + item.cnt11T;
						data15T["col" + day] = Util.nvl(Util.formatNumber(item.cnt15T), 0);
						data15T["subtotal"] = data15T["subtotal"] + item.cnt15T;
						data25T["col" + day] = Util.nvl(Util.formatNumber(item.cnt25T), 0);
						data25T["subtotal"] = data25T["subtotal"] + item.cnt25T;
						data20FT["col" + day] = Util.nvl(Util.formatNumber(item.cnt20FT), 0);
						data20FT["subtotal"] = data20FT["subtotal"] + item.cnt20FT;
						data40FT["col" + day] = Util.nvl(Util.formatNumber(item.cnt40FT), 0);
						data40FT["subtotal"] = data40FT["subtotal"] + item.cnt40FT;
						dataETC["col" + day] = Util.nvl(Util.formatNumber(item.cntETC), 0);
						dataETC["subtotal"] = dataETC["subtotal"] + item.cntETC;
						dataAmtSum["col" + day] = Util.nvl(Util.formatNumber(amtSum), 0);
						dataAmtSum["subtotal"] = dataAmtSum["subtotal"] + amtSum;
						data1TAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt1T), 0);
						data1TAmt["subtotal"] = data1TAmt["subtotal"] + item.amt1T;
						data2T5Amt["col" + day] = Util.nvl(Util.formatNumber(item.amt2T5), 0);
						data2T5Amt["subtotal"] = data2T5Amt["subtotal"] + item.amt2T5;
						data3T5Amt["col" + day] = Util.nvl(Util.formatNumber(item.amt3T5), 0);
						data3T5Amt["subtotal"] = data3T5Amt["subtotal"] + item.amt3T5;
						data5TAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt5T), 0);
						data5TAmt["subtotal"] = data5TAmt["subtotal"] + item.amt5T;
						data5TAAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt5A), 0);
						data5TAAmt["subtotal"] = data5TAAmt["subtotal"] + item.amt5A;
						data11TAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt11T), 0);
						data11TAmt["subtotal"] = data11TAmt["subtotal"] + item.amt11T;
						data15TAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt15T), 0);
						data15TAmt["subtotal"] = data15TAmt["subtotal"] + item.amt15T;
						data25TAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt25T), 0);
						data25TAmt["subtotal"] = data25TAmt["subtotal"] + item.amt25T;
						data20FTAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt20FT), 0);
						data20FTAmt["subtotal"] = data20FTAmt["subtotal"] + item.amt20FT;
						data40FTAmt["col" + day] = Util.nvl(Util.formatNumber(item.amt40FT), 0);
						data40FTAmt["subtotal"] = data40FTAmt["subtotal"] + item.amt40FT;
						dataETCAmt["col" + day] = Util.nvl(Util.formatNumber(item.amtETC), 0);
					...
					for (var item, i=0; item = response.data[i]; i++) {
						var day = item.sDate.substring(10,8);
						var orderSum = data["col" + day];
						if(item.allocCnt != "0"){
		 					allocData["col" + day] = Util.formatNumber(item.allocCnt + "</br>" + (item.allocCnt / orderSum * 100).toFixed(2) + "%");
						}else{
							allocData["col" + day] = Util.formatNumber(item.allocCnt + "</br>" + "0.00%");
						}
						
	 					if(item.notAllocCnt != "0"){
		 					notAllocData["col" + day] = Util.formatNumber(item.notAllocCnt + "<br>" + (item.notAllocCnt / orderSum * 100).toFixed(2) + "%");
	 					}else{
	 						notAllocData["col" + day] = Util.formatNumber(item.notAllocCnt + "<br>" + "0.00%");
	 					}
					}
		...
		excel: {
			fileName: "배차및운송비(일)(" + new Date().yyyymmdd() + ").xlsx",
