...
$(document).ready(function(){
	...
	Util.setSelectBox("/contents/basic/data/userNameList.do", "userId", {deptId:'${sessionScope.userInfo.deptId}'}, "userId", "userName", "", "선택하세요");
...
function goList(){
	...
	for(var i = 1; i<=lastDay; i++) {
		...
		if (month.indexOf("0") == 0 ) month = month.substring(1, 2);
		str = str.substring(str.length, str.length-2);
		colName = "col" + str;
		title = month + "." + i.toString();
		data = {field: colName, title: title, width: 100, 
			attributes: { 
				style: "text-align: right" 
			}
		};
	...
	$("#grid").kendoGrid({
		dataSource : {
			transport : {
				read : {
					url : "/contents/monitor/data/profitDeptMonth.do",
					type : "post",
					dataType : "json",
				...
				data : function(response) {
					...
					for(var h=0; h<stateArr.length; h++) {
						for(var i=0, item; item=response.dept[i]; i++) {
							data = {"deptName" : item.deptName, "deptId" : item.deptId, "userName" : item.userName, "type" : stateArr[h], "colSum" : "0"};
							countSum = 0;	
							countSum2 = 0;
							countSum3 = 0;
							countSum4 = 0;
							...
							for(var j=0, item2; item2=response.data[j]; j++) {
								profitCharge = 0; 
								if(item.userId == item2.userId && item.deptId == item2.deptId) {
									var day = item2.allocDate.substring(10, 8);
									sellAmt += item2.sellCharge;
									buyAmt += item2.buyCharge;
									profitCharge = parseInt(item2.sellCharge, 0) - parseInt(item2.buyCharge, 0);
									profitAmt += profitCharge; 
									if(data["type"] == "매출액") data["col" + day] = Util.nvl(Util.formatNumber(item2.sellCharge),"0");
									if(data["type"] == "매입액") data["col" + day] = Util.nvl(Util.formatNumber(item2.buyCharge),"0");
									if(data["type"] == "한계이익") data["col" + day] = Util.nvl(Util.formatNumber(profitCharge),"0");
									if(data["type"] == "한계이익율" && profitCharge != 0) data["col" + day] = (profitCharge / item2.sellCharge * 100).toFixed(1) + "%";
								}
							}
							...
							if(data["type"] == "한계이익율") {
								if(isNaN((profitAmt / sellAmt * 100).toFixed(1)) == false){
									data["colSum"] = (profitAmt / sellAmt * 100).toFixed(1) + "%";
								}else{
									data["colSum"] = "0.0%";
								}
								profitPercentageList.push(data);	
							}
						}
						
					}

					sumData = {"deptName" : "합계", "userName" : "합계", "type" : "매출액", "colSum" : "0", "deptId" : "sum"}
					sumData2 = {"deptName" : "합계", "userName" : "합계", "type" : "매입액", "colSum" : "0", "deptId" : "sum"}
					sumData3 = {"deptName" : "합계", "userName" : "합계", "type" : "한계이익", "colSum" : "0", "deptId" : "sum"}
					sumData4 = {"deptName" : "합계", "userName" : "합계", "type" : "한계이익율", "colSum" : "0", "deptId" : "sum"}
					for(var k=0;k<32;k++){
						var str = "0" + k.toString();
						str = str.substring(str.length, str.length-2);
						sumData["col" + str] = "0";
					...
					for(var i=0, item; item=response.total[i]; i++) {
						var day = item.allocDate.substring(10, 8);
						var profitCharge = parseInt(item.sellCharge, 0) - parseInt(item.buyCharge, 0);
						sumData["col" + day] = Util.nvl(Util.formatNumber(item.sellCharge),"0");
						sumData2["col" + day] = Util.nvl(Util.formatNumber(item.buyCharge),"0");
						sumData3["col" + day] = Util.nvl(Util.formatNumber(profitCharge),"0");
						if(isNaN((profitCharge / item.sellCharge * 100).toFixed(1)) == false){
							sumData4["col" + day] = (profitCharge / item.sellCharge * 100).toFixed(1) + "%";
						}else{
							sumData4["col" + day] = "0.0%"
						}
					...
					if(isNaN((sumData3["colSum"] / sumData["colSum"] * 100).toFixed(1)) == false){
						sumData4["colSum"] = (sumData3["colSum"] / sumData["colSum"] * 100).toFixed(1) + "%";;
					}else{
						sumData4["colSum"] = "0.0%"
					}
					...
					dataList = dataList.concat(sellList).concat(buyList).concat(profitList).concat(profitPercentageList);
  	...
  	for (var i = 0; i < rows.length; i++) {
		...
		stateCells = row.children().eq(0)
   		...
   		if(userNameCells[0].innerText == '합계') {
   			userNameCells.addClass(gridAddClass(userNameCells.prevObject[0].innerText+"-"+userNameCells.prevObject[4].innerText, 'colspan'))
   		}
		stateCells.addClass(gridAddClass(stateCells[0].innerText, "stateCells"));
		deptIdCells.addClass(gridAddClass(deptIdCells.prevObject[0].innerText+"-"+deptIdCells.prevObject[4].innerText, "deptId"));
