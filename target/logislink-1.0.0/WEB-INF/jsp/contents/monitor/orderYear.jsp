	...
	<div class="contents">
		<div id="group-list" class="cont-wrapper-page-grid">
			<form id="fSearch" class="date-bnt" method="post">
				...
				</div><div class="input-group input-group-sm col radio-or-checkBox">
...
function goList(){
	...
	$("#fSearch input[name=orderState]").each(function(){
		if($(this).is(":checked")){
			var value = $(this).val();
			orderState += value + ",";
			if (value == "05")	stateCategory += "도착,";
			else if (value == "01") stateCategory += "진행,";
			else if (value == "00") stateCategory += "접수,";
			else  stateCategory += "취소,";
		}	
	});
	...
	$("#grid").kendoGrid({
		dataSource : {
			transport : {
				read : {
					url : "/contents/monitor/data/orderYear.do",
					type : "post",
					dataType : "json",
				...
				data : function(response) {
					...
					for(var h=0; h<stateArr.length; h++) {
						for(var i=0, item; item=response.user[i]; i++) {
							data = {"deptName" : item.deptName, "deptId" : item.deptId, "userName" : item.userName, "type" : stateArr[h], "orderState" : stateArr[h], "colSum" : "0"};						
							countSum = 0;		
							...
							for(var j=0, item2; item2=response.data[j]; j++) {
								if(item.userId == item2.userId && stateArr[h] == item2.orderState) {
									var month = item2.regdate.substring(7, 5);
									data["col" + month] = item2.orderCount;
									countSum += item2.orderCount;
								}
							}
							data["colSum"] = countSum;
							if(data["type"] == "전체") totalList.push(data);
							if(data["type"] == "도착") finishList.push(data);
							if(data["type"] == "진행") allocList.push(data);
							if(data["type"] == "접수") receiptList.push(data);
							if(data["type"] == "취소") cancelList.push(data);
							if(data["type"] == "입력지연") delayList.push(data);
						}

						sumData = {"deptName" : "합계", "userName" : "합계", "type" : stateArr[h], "orderState" : stateArr[h], "deptId" : "sum"}
						for(var k=0;k<32;k++){
							var str = "0" + k.toString();
							str = str.substring(str.length, str.length-2);
							sumData["col" + str] = "0";
						...
						for(var i=0, item; item=response.total[i]; i++) {
							...
							if(orderState == item.orderState) {
								var month = item.regdate.substring(7, 5);
								sumData["col" + month] = item.orderCount;
					...
					dataList = totalList.concat(finishList).concat(allocList).concat(receiptList).concat(cancelList).concat(delayList);
