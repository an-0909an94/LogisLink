			...
			schema : {
				data : function(response) {
						...
						for(var i=0, item; item=response.dept[i]; i++) {
							...
							for(var j=0, item2; item2=response.data[j]; j++) {
								...
								if(item.userId == item2.userId && item.deptId == item2.deptId) {
									var month = item2.allocDate.substring(7, 5);
									...
									if(data["type"] == "매출액") data["col" + month] = Util.nvl(Util.formatNumber(item2.sellCharge),"0");
									if(data["type"] == "매입액") data["col" + month] = Util.nvl(Util.formatNumber(item2.buyCharge),"0");
									if(data["type"] == "한계이익") data["col" + month] = Util.nvl(Util.formatNumber(profitCharge),"0");
									if(data["type"] == "한계이익율" && profitCharge != 0) data["col" + month] = (profitCharge / item2.sellCharge * 100).toFixed(1) + "%";
								}
							}
					...
					for(var i=0, item; item=response.total[i]; i++) {
						var month = item.allocDate.substring(7, 5);
						var profitCharge = parseInt(item.sellCharge, 0) - parseInt(item.buyCharge, 0);
						sumData["col" + month] = Util.nvl(Util.formatNumber(item.sellCharge),"0");
						sumData2["col" + month] = Util.nvl(Util.formatNumber(item.buyCharge),"0");
						sumData3["col" + month] = Util.nvl(Util.formatNumber(profitCharge),"0");
						if(isNaN((profitCharge / item.sellCharge * 100).toFixed(1)) == false){
							sumData4["col" + month] = (profitCharge / item.sellCharge * 100).toFixed(1) + "%";
						}else{
